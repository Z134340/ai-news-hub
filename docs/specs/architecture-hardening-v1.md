...
| workflow 結構與語法 | 舊 HEAD deploy run `35456640348`、rollback run `35456640876` 均為 0 jobs 的 workflow file failure；`actionlint v1.7.12` 指出 deploy 1 處、rollback 3 處 job-level `env` 不允許 `runner.temp`。修復後 receipt 仍位於 runner temp，不進 workspace／`dist/`；本次以官方 checksum 驗證固定版本後重跑全部 workflows 為 0 error，workflow 防漂移測試 4/4。GitHub run `35515416808` 對完整 SHA `098ea71384d1929fcbb48293730a0d1ce3c48157` 成功且 semantic validation step 通過；該 SHA 只有 selftest run、GitHub deployments 與 artifacts 均為空，repository workflow configuration 完成 |
