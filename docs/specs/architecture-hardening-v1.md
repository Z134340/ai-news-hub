...
| 驗收 | 實際結果／fixture 證據 |
|---|---|
| 全分類、單分類失敗、全部失敗 | `test_category_quality.py`：合格分類前進；失敗分類精確沿用自己 LKG；整體比例不能代替分類決策 |
...
| Python 全套 | 101/101：AH-02 43、AH-01 schema／migration／書籤相容 31、robustness 27 |
...
Re-scan：分類閘及補跑繞過問題已處理；只讀複查發現的「中斷重跑覆蓋中間更新」與「無 LKG 時未排程遺失 checked」均已修復並納入回歸。AH-03 release manifest／版本化讀取仍未開始；AH-04 之後項目、L3 關鍵路徑、Firebase v3、通知時機與 learning 人工邊界均未變。下一個 Session 只做 AH-03。
