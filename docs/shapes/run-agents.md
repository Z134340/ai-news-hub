...
| Self-test | S-1 執行檔存在（16 支）／S-2 無 `--promote`／S-2b 無 `--out-dir`／S-3 `.gitignore` 含 `data/agent/.preview/`／S-4 status 檔在 preview 內／S-5 `BUDGET_SEC ≥ MODEL_STEP_TIMEOUT×5`／S-6 無 step 動 `memory/`／S-6b~S-6i 轉呼叫子腳本 `--self-test`（S-6i = pull-feedback、S-6j = build-category-metrics）／S-7 `bash -n`／S-8 model artifact 集合 == `"brief-latest.json roadmap.json trend-assessment.json "`／S-8b model runner 不得用裸 `run_step`／S-8c、S-8d observer 09/10 精確行／S-9a~f `guard_verdict` 真值表 |
