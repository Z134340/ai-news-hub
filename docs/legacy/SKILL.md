...
4. **時效性**：每日類別優先 24 小時內，次之 3 日內，超過 7 天不收錄。累積類別（模型/教學）搜尋近 3 個月。
...
│   ├── health-check.yml
│   ├── keep-alive.yml
│   └── notify.yml                   ← push 後自動建 Issue 通知
├── scripts/
│   ├── run-daily.sh                ← 每日擷取主腳本（含 DOW 排程）
│   ├── validate.py                 ← 八步驟 URL 驗證（含標題一致性）
│   ├── extract-json.py             ← Claude 回應 JSON 提取
│   ├── merge-stack.py              ← 模型/教學 累積合併（週一執行）
...
**優先主題（排序固定）：** 🥇 LLM-as-a-Judge / LLM 評估 → 🥇 Agent 評估 / AgentBench → 🥇 AI Agent / Multi-agent 研究 → 🥈 LLM 新架構/訓練 → 🥉 其他頂尖論文
**搜尋策略：** 先用具體 arxiv Query 搜尋優先主題，再擴大搜尋其他論文
**欄位：** title, authors, institution, venue, date, summary(4-5句技術摘要含數字), url, field, impact

### 2. 🔥 全球熱門 AI 新聞 Top 20 (`topnews`)
**來源：** TechCrunch, The Verge, VentureBeat, Wired, MIT Tech Review, Bloomberg, Reuters, Ars Technica, The Information, Hacker News, Latent Space, Import AI, The Gradient, Arxiv Sanity, 各大 AI 公司 Blog, Hugging Face, Papers with Code, AI Feed
**優先主題（排序固定）：** 🥇 AI Agent/Agentic/Multi-agent → 🥇 LLM 新模型發布 → 🥇 技術大廠新產品/模型 → 🥈 AI 資安 → 🥉 其他
**搜尋策略：** 先以具體 Query 搜尋優先主題，再擴大搜尋其他新聞
**欄位：** title, source, date, domain, model_area, summary(4-6句), highlights, url

### 3. 🇹🇼 台灣 AI 熱議 Top 30 (`taiwan`)
...
**語言：** 繁中+英文搜尋，繁中摘要
**優先主題（排序固定）：** 🥇 AI Agent/Agentic/Multi-agent 在台灣 → 🥇 LLM 模型發布/更新 → 🥇 技術大廠新產品/模型在台灣動態 → 🥈 AI 資安 → 🥉 其他台灣 AI 產業
**搜尋策略：** 先以具體中英文 Query 搜尋優先主題，再擴大搜尋其他新聞
**欄位：** title(繁中), source, date, topic, summary(繁中), relevance, discussion, url

### 4. 🇨🇳 中國 AI 熱議 Top 20 (`china`)
**來源：** 机器之心, 量子位, 36氪, Baidu/Alibaba/Tencent/ByteDance, SenseTime, Huawei, DeepSeek, Moonshot, Baichuan, 01.AI, MiniMax, Zhipu, Tsinghua, Zhihu, MIIT/MOST
**語言：** 簡中+英文
**優先主題（排序固定）：** 🥇 AI Agent/智能體/Multi-agent → 🥇 中國 LLM 新模型（DeepSeek/Qwen/GLM/Baichuan/Yi 等） → 🥇 技術大廠新產品/模型 → 🥈 AI 資安 → 🥉 其他中國 AI 動態
**搜尋策略：** 先以具體中英文 Query 搜尋優先主題，再擴大搜尋其他新聞
**欄位：** title, source, date, company, topic, summary, discussion, url

### 5. 🇺🇸 美國 AI 熱議 Top 30 (`usa`)
**來源：** OpenAI, Google/DeepMind, Anthropic, Meta, Microsoft, Apple, AWS, NVIDIA, xAI, Cohere, AI Business, AI Magazine, The Rundown AI, US AI 政策(White House/Congress/FTC/NIST), Stanford HAI, MIT, X/Reddit/HN, VentureBeat, TechCrunch, The Information
**優先主題（排序固定）：** 🥇 AI Agent/Agentic/Multi-agent → 🥇 LLM 新模型發布 → 🥇 技術大廠新產品/模型 → 🥈 AI 資安 → 🥉 其他美國 AI 動態
**搜尋策略：** 先以具體 Query 搜尋優先主題（各大廠 blog + AI agent + LLM + 資安），再擴大搜尋
...
- watchdog 600 秒（10 分鐘）：確保最壞情況下 8 類 × 20.5 分 = 19:20 前完成
...
- 評分規則：
  ...
  · 0 < score < 0.3 → needs_review（標記但不移除）
  · score = 0（完全無法對應）→ verified: false → 移除
- needs_review 計入 pass_rate（不扣分）
- TITLE_CHECK_RELAXED_DOMAINS（arxiv, medium, 中文媒體等）跳過標題比對
...
- 新聞/論文：today-90 至 today+1（寬鬆，涵蓋 consulting 報告等非即時內容）
- 模型 release_date：允許未來日期，最多 2 年前
```python
def check_date(date_str, allow_future=False, allow_empty=False):
    if not date_str:
        return allow_empty
    try:
    ...
    if allow_future:
        return True
    return (today - timedelta(days=7)) <= d <= today
```
...
- 右側（同一列，永遠靠右）：狀態膠囊 + 驗證率 + 更新時間 + 搜尋
- `.hdr-inner{width:100%}` 確保容器填滿 header 全寬；`.hdr-right{margin-left:auto}` 確保狀態列固定靠右端
  · 🟢 綠色脈動 = 今日資料已就緒 + 驗證率 ≥ 90%
  · 🟡 黃色 = 今日資料已就緒但驗證率 < 90%
  · 🔴 紅色 = 資料非今日（排程可能沒跑）
