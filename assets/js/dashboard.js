...
/* ======== 圖表分類色票（保留多色及文字／線型，使用全站語意 tokens） ======== */
const DASH_CATS = [
  { key:'papers',     label:'論文',     color:'var(--ac)' },
  { key:'topnews',    label:'全球熱門', color:'var(--amber)' },
  { key:'taiwan',     label:'台灣',     color:'var(--green)' },
  { key:'china',      label:'中國',     color:'var(--red)' },
  { key:'usa',        label:'美國',     color:'var(--blue)' },
  { key:'techtrends', label:'技術趨勢', color:'var(--purple)' },
  { key:'governance', label:'科技治理', color:'var(--pink)' },
  { key:'tutorials',  label:'工具教學', color:'var(--amber)' },
  { key:'courses',    label:'課程證照', color:'var(--green)' },
  { key:'models',     label:'模型快訊', color:'var(--ac)' },
];

/* 上游代理人的列舉值 → 中文。值域外的一律原樣顯示，不猜。 */
const DASH_STAGE = { emerging:'萌芽', accelerating:'加速', plateau:'高原', declining:'衰退' };
const DASH_STAGE_COLOR = { emerging:'var(--cyan)', accelerating:'var(--green)', plateau:'var(--amber)', declining:'var(--red)' };
/* 階段徽章的三重冗餘編碼：明度階（bg alpha）＋ 邊框線型 ＋ 圖示。
   色相不是唯一線索；淺色主題仍保留文字、圖示與線型供辨識。 */
...
const DASH_CLUSTER_STYLE = [
  { color:'var(--ac)', dash:'' },
  { color:'var(--green)', dash:'6 3' },
  { color:'var(--amber)', dash:'2 3' },
  { color:'var(--pink)', dash:'10 3 2 3' },
  { color:'var(--cyan)', dash:'1 4' },
  { color:'var(--red)', dash:'8 4' },
];
...
const DASH_CONF_COLOR = { verified:'var(--green)', snippet_inference:'var(--amber)', unverified:'var(--red)' };
const DASH_STATUS_META = {
  fresh:   { label:'最新公開',       title:'Public 已對齊',       color:'var(--green)', ico:'check' },
  pending: { label:'等待 Owner 審核', title:'Preview 尚未發布',    color:'var(--amber)', ico:'clock' },
  stale:   { label:'資料過期',       title:'新鮮度未達門檻',       color:'var(--red)', ico:'alert' },
  blocked: { label:'狀態阻擋',       title:'證據或契約阻擋',       color:'var(--red)', ico:'shield' },
  missing: { label:'狀態尚未發布',    title:'無法判斷公開資料狀態', color:'var(--tx3)', ico:'file' },
  invalid: { label:'狀態契約無效',    title:'拒絕呈現不可信狀態',   color:'var(--red)', ico:'alert' },
};
...
async function dashFetch(url) {
  if(url==='data/latest.json') {try{return (await readLatestRelease()).document;}catch{return null;}}
  try {
    ...
    return dashSection('shield', meta.color, '系統狀態', meta.label,
      `<div class="dss-card dss-missing" data-system-status="missing" role="status">
        <div class="dss-head"><span class="dss-state" style="color:${meta.color}">${svg(meta.ico, 14, meta.color)}${meta.title}</span>
          <span class="dss-badge" style="color:${meta.color};border-color:color-mix(in srgb, ${meta.color} 33.33%, transparent)">${meta.label}</span></div>
...
function dashBumpBody() {
  ...
  const grid = b.rows.map((_, k) =>
    `<line x1="${PL}" x2="${W - PR + 4}" y1="${y(k + 1)}" y2="${y(k + 1)}" stroke="var(--bd)" stroke-width="1"/>
     <text x="${PL - 9}" y="${y(k + 1) + 3.5}" text-anchor="end" font-size="10" fill="var(--tx3)">${k + 1}</text>`
  ).join('');

  const heads = b.months.map((m, j) =>
    `<text x="${x(j).toFixed(1)}" y="16" text-anchor="middle" font-size="11" fill="var(--tx2)">${m.label}</text>
     <text x="${x(j).toFixed(1)}" y="28" text-anchor="middle" font-size="9" fill="var(--tx3)">${m.obs} 觀測日</text>`
  ).join('');
  ...
  const nodes = b.rows.map(r => r.ranks.map((rk, j) => {
    ...
    return `<circle cx="${x(j).toFixed(1)}" cy="${y(rk)}" r="4.5" fill="${r.style.color}" stroke="var(--sf)" stroke-width="1.5"><title>${esc(tip)}</title></circle>
            <text x="${x(j).toFixed(1)}" y="${y(rk) - 9}" text-anchor="middle" font-size="9.5" fill="var(--tx2)">${val(c)}</text>`;
  }).join('')).join('');
  ...
  const labels = b.rows.map(r => {
    ...
    const ly = y(lastRank);
    const tri = d === 0
      ? `<line x1="${W - PR + 8}" x2="${W - PR + 16}" y1="${ly}" y2="${ly}" stroke="var(--tx3)" stroke-width="1.6"/>`
      : (d > 0
        ? `<path d="M${W - PR + 8} ${ly + 4} L${W - PR + 12} ${ly - 3} L${W - PR + 16} ${ly + 4} Z" fill="var(--green)"/>`
        : `<path d="M${W - PR + 8} ${ly - 4} L${W - PR + 12} ${ly + 3} L${W - PR + 16} ${ly - 4} Z" fill="var(--red)"/>`);
    return `${tri}
      <text x="${W - PR + 21}" y="${ly + 3.5}" font-size="10.5" fill="var(--tx)">${esc(r.title)}</text>
      <text x="${W - 4}" y="${ly + 3.5}" text-anchor="end" font-size="10" fill="${d > 0 ? 'var(--green)' : (d < 0 ? 'var(--red)' : 'var(--tx3)')}">${d === 0 ? '持平' : (d > 0 ? '+' : '') + d}</text>`;
  }).join('');
...
function dashStageBadge(stage) {
  const color = DASH_STAGE_COLOR[stage] || 'var(--tx3)';
  const zh = DASH_STAGE[stage] || String(stage || '—');
  const enc = DASH_STAGE_ENC[stage];
  if (!enc) return `<span class="dm-stage" style="border-color:color-mix(in srgb, ${color} 33.33%, transparent);background:color-mix(in srgb, ${color} 7.84%, transparent);color:${color}">${esc(zh)}</span>`;
  return `<span class="dm-stage" style="border-style:${enc.line};border-width:${enc.line === 'double' ? '3px' : '2px'};border-color:color-mix(in srgb, ${color} 40.0%, transparent);background:color-mix(in srgb, ${color} ${parseInt(enc.a,16)/255*100}%, transparent);color:${color}">${svg(enc.ico, 11, color)}${zh}</span>`;
}
...
function dashMatrixBlock() {
  ...
  if (!tl || !Array.isArray(tl.clusters) || !tl.clusters.length) {
    return dashNoTimeline('cpu', 'var(--ac)', '主題矩陣');
  }
  ...
  const body = rows.map(r => {
    ...
    const relColor = r.rel === null ? 'var(--tx3)' : (r.rel > 0.02 ? 'var(--green)' : (r.rel < -0.02 ? 'var(--red)' : 'var(--tx2)'));
    const dup = typeof sy.duplicate_title_ratio === 'number' ? sy.duplicate_title_ratio : null;
    const dupColor = dup === null ? 'var(--tx3)' : (dup >= 0.5 ? 'var(--red)' : (dup >= 0.3 ? 'var(--amber)' : 'var(--tx2)'));
  ...
  const meta = `<div class="dash-meta">
    ${badge('var(--ac)', `${w.observed_days ?? '—'} / ${w.calendar_days ?? '—'} 觀測日`)}
    ${badge('var(--amber)', `連續性 ${dashNum(w.continuity, 4)}`)}
    ${badge('var(--red)', `缺 ${(w.missing_dates || []).length} 天`)}
    ${base ? badge('var(--tx3)', `語料基準 ${base.toFixed(4)}`) : ''}
    ${tl.mode ? badge('var(--tx3)', `模式 ${esc(String(tl.mode))}`) : ''}
    ${tl.advisory ? badge('var(--purple)', '僅供參考，不進生產寫入') : ''}
  </div>`;
  ...
  const defs = `<svg width="0" height="0" style="position:absolute" aria-hidden="true"><defs>
    <pattern id="dashGap" width="4" height="4" patternUnits="userSpaceOnUse">
      <rect width="4" height="4" fill="color-mix(in srgb, var(--red) 4.5%, transparent)"/>
      <circle cx="1" cy="1" r="0.75" fill="color-mix(in srgb, var(--red) 42%, transparent)"/>
    </pattern></defs></svg>`;

  return dashSection('cpu', 'var(--ac)', '主題矩陣', `${rows.length} 個叢集 × 90 天`,
...
function dwSuff(o) { return o && o.sufficient === false ? badge('var(--amber)', '樣本不足') : ''; }
...
function dashDrawerBody(c) {
  ...
  /* ① 階段判讀 */
  const c1 = dwCard('flame', 'var(--amber)', '階段判讀', a ? `
    <div class="dw-badges">${dashStageBadge(a.stage)}
      ${a.syndication_call ? badge('var(--tx3)', DASH_SYND[a.syndication_call] || esc(String(a.syndication_call))) : ''}
      ${typeof a.confidence === 'number' ? badge('var(--ac)', `信心 ${a.confidence.toFixed(2)}`) : ''}
      ${a.security_flag ? badge('var(--red)', '安全旗標') : ''}</div>
    ...
    ${(a.rubric_hits || []).length ? `<div class="dw-tags">${a.rubric_hits.map(x => badge('var(--tx3)', esc(String(x)))).join('')}</div>` : ''}
    ${(a.gate_notes || []).length ? `<div class="dash-note">閘門註記：${esc(a.gate_notes.join('；'))}</div>` : ''}
  ` : '<div class="empty">此叢集沒有 TrendAnalyst 判讀。</div>');

  /* ② 路線預測 */
  const c2 = dwCard('rocket', 'var(--cyan)', '路線預測', r ? `
    <div class="dw-badges">${badge('var(--cyan)', DASH_TRAJ[r.trajectory] || esc(String(r.trajectory || '—')))}
      ${r.horizon ? badge('var(--tx3)', DASH_HORIZON[r.horizon] || esc(String(r.horizon))) : ''}
      ${typeof r.confidence === 'number' ? badge('var(--ac)', `信心 ${r.confidence.toFixed(2)}`) : ''}</div>
  ...
  /* ③ 量能指標 */
  const c3 = dwCard('chart', 'var(--ac)', '量能指標', `
  ...
  const bars = (sc.top_sources || []).map(s => `
    <div class="dbd-row"><span class="dbd-k">${esc(String(s.source || ''))}</span>
      <span class="dbd-track"><span class="dbd-fill" style="width:${((s.share || 0) * 100).toFixed(1)}%;background:${(s.share || 0) >= 0.25 ? 'var(--red)' : 'var(--ac)'}"></span></span>
      <span class="dbd-v">${dashPct((s.share || 0) * 100)}%</span></div>`).join('');
  const c4 = dwCard('globe', 'var(--green)', '來源結構', `
  ...
  const c5 = dwCard('users', 'var(--pink)', '轉載證據', `
  ...
  const c6 = dwCard('shield', 'var(--purple)', '落地約束', `
    ${bl ? `<ul class="dw-ul dw-bl">${bl}</ul>` : '<div class="empty">此叢集沒有列出落地約束（RM-6 允許空集合，不是漏填）。</div>'}
    ${(r?.rubric_hits || []).length ? `<div class="dw-tags">${r.rubric_hits.map(x => badge('var(--tx3)', esc(String(x)))).join('')}</div>` : ''}
...
function dashOpsBlock() {
  ...
  return `<details class="dash-ops">
    <summary><span class="dop-l">${svg('shield', 14, 'var(--tx3)')}管線健康度</span>
...
function dashKpis() {
  ...
  return `<div class="dash-kpis">
    ${cell('var(--ac)','calendar','存檔涵蓋', covered + ' 天', `${first} → ${last}（區間 ${span} 天，覆蓋 ${dashPct(covered / span * 100)}%）`)}
    ${cell('var(--green)','news','最新一日', dashDayCount(latest) + ' 筆', `${latest.date}${latest._cold ? '（冷封存）' : '（熱層）'}`)}
    ${cell('var(--amber)','chart','日均筆數', avg + ' 筆', `累計 ${total} 筆`)}
    ${cell('var(--pink)','shield','平均驗證', pr === null ? '—' : pr + '%', prs.length ? `${prs.length} 天有驗證紀錄` : '無驗證紀錄')}
  </div>`;
}
...
function dashTimelineBlock() {
  const days = DASH.days;
  if (!days.length) {
    return dashSection('calendar', 'var(--ac)', '時間軸', '每日產出量與缺日',
  ...
  const bars = axis.map((d, i) => {
    ...
    if (!e) {
      ...
      return `<rect x="${x.toFixed(2)}" y="${(PT + ih - 3).toFixed(2)}" width="${bw.toFixed(2)}" height="3" rx="1" fill="var(--red)" opacity="0.45"><title>${d}　無資料</title></rect>`;
    }
    const c = dashDayCount(e);
    const h = Math.max(2, (c / max) * ih);
    const y = PT + ih - h;
    const fill = e._cold ? 'var(--purple)' : 'var(--ac)';
  ...
  const grid = [0, 0.5, 1].map(t => {
    const y = PT + ih - t * ih;
    return `<line x1="${PL}" x2="${W - PR}" y1="${y.toFixed(1)}" y2="${y.toFixed(1)}" stroke="var(--bd)" stroke-width="1"/>
            <text x="${PL - 5}" y="${(y + 3).toFixed(1)}" text-anchor="end" font-size="9" fill="var(--tx3)">${Math.round(max * t)}</text>`;
  }).join('');

  const ticks = [0, Math.floor(axis.length / 2), axis.length - 1]
    .filter((v, i, a) => a.indexOf(v) === i)
    .map(i => `<text x="${(PL + slot * i + slot / 2).toFixed(1)}" y="${H - 6}" text-anchor="${i === 0 ? 'start' : (i === axis.length - 1 ? 'end' : 'middle')}" font-size="9" fill="var(--tx3)">${dashMd(axis[i])}</text>`)
    .join('');

  const legend = `<div class="dash-legend">
    <span class="dl-item"><i style="background:var(--ac)"></i>熱層（repo）</span>
    <span class="dl-item"><i style="background:var(--purple)"></i>冷封存（Firestore）</span>
    <span class="dl-item"><i style="background:var(--red);opacity:.45"></i>缺日 ${missing.length} 天</span>
  </div>`;
  ...
  return dashSection('calendar', 'var(--ac)', '時間軸', `${axis.length} 天區間、${days.length} 天有資料`, chart);
}
...
function dashTrendBlock() {
  ...
  if (!t || !Array.isArray(t.clusters) || !t.clusters.length) {
    return dashSection('chart', 'var(--purple)', '主題趨勢', '尚未產出',
  ...
  const staleBadge = (staleDays !== null && staleDays > 1)
    ? badge('var(--amber)', `語料日 ${srcDate}，落後今日 ${staleDays} 天`)
    : badge('var(--green)', `語料日 ${srcDate || '—'}`);

  const meta = `<div class="dash-meta">
    ${staleBadge}
    ${badge('var(--ac)', `${t.clusters.length} 個叢集`)}
    ${t.mode ? badge('var(--tx3)', `模式 ${esc(t.mode)}`) : ''}
    ${t.advisory ? badge('var(--purple)', '僅供參考，不進生產寫入') : ''}
    ${DASH.assessment ? badge('var(--green)', '含階段判讀') : badge('var(--tx3)', '無階段判讀')}
    ${DASH.roadmap ? badge('var(--green)', '含路線預測') : badge('var(--tx3)', '無路線預測')}
  </div>`;
  ...
  return dashSection('chart', 'var(--purple)', '主題趨勢', `依綜合分數排序`, meta + cards);
}
...
function dashTrendCard(c, idx, maxScore, a, r) {
  ...
  const stage = a?.stage;
  const stageColor = DASH_STAGE_COLOR[stage] || 'var(--tx3)';
  ...
  const breakdown = c.score_breakdown ? Object.entries(c.score_breakdown).map(([k, v]) => {
    ...
    return `<div class="dbd-row"><span class="dbd-k">${esc(k)}</span>
      <span class="dbd-track"><span class="dbd-fill" style="width:${(pv * 100).toFixed(1)}%;background:${pv >= 0.8 ? 'var(--green)' : (pv >= 0.5 ? 'var(--amber)' : 'var(--red)')}"></span></span>
  ...
  const cats = (c.categories || []).slice(0, 8).map(x => {
    ...
    return cat ? badge(cat.color, cat.label) : badge('var(--tx3)', esc(String(key)));
  }).join('');
  ...
  const stageRow = a ? `<div class="dash-agent-row" style="border-color:color-mix(in srgb, ${stageColor} 13.33%, transparent);background:color-mix(in srgb, ${stageColor} 3.92%, transparent)">
      <div class="dar-head">${svg('flame', 12, stageColor)}<span style="color:${stageColor}">階段判讀：${DASH_STAGE[stage] || esc(String(stage || '—'))}</span>
        ${a.syndication_call ? badge('var(--tx3)', DASH_SYND[a.syndication_call] || esc(String(a.syndication_call))) : ''}
        ${typeof a.confidence === 'number' ? badge('var(--ac)', `信心 ${a.confidence.toFixed(2)}`) : ''}</div>
      ${a.headline_zh ? `<p class="dar-text">${esc(a.headline_zh)}</p>` : ''}
      ${scores ? `<div class="da-metrics">${scores}</div>` : ''}
    </div>` : '';

  const rmRow = r ? `<div class="dash-agent-row" style="border-color:color-mix(in srgb, var(--cyan) 13.33%, transparent);background:color-mix(in srgb, var(--cyan) 3.92%, transparent)">
      <div class="dar-head">${svg('rocket', 12, 'var(--cyan)')}<span style="color:var(--cyan)">路線預測：${DASH_TRAJ[r.trajectory] || esc(String(r.trajectory || '—'))}</span>
        ${r.horizon ? badge('var(--tx3)', DASH_HORIZON[r.horizon] || esc(String(r.horizon))) : ''}
        ${typeof r.confidence === 'number' ? badge('var(--ac)', `信心 ${r.confidence.toFixed(2)}`) : ''}</div>
    ...
    <div class="card-row">
      ...
      <div class="card-body">
        ...
        <div class="card-badges">
          ${badge('var(--ac)', `證據 ${c.evidence_count || (c.evidence || []).length} 則`)}
          ${(c.sources && c.sources.length) ? badge('var(--tx3)', `${c.sources.length} 個來源`) : ''}
          ${cats}
        </div>
        ${breakdown ? `<div class="dash-breakdown">${breakdown}</div>` : ''}
        ${c.why_now ? infoBlock('flame', '為何是現在', c.why_now) : ''}
        ${c.financial_implication ? infoBlock('building', '金融業意涵', c.financial_implication) : ''}
        ${stageRow}${rmRow}
      </div>
    </div>
  </div>`;
...
function dashBriefBlock() {
  ...
  if (!b || !Array.isArray(b.highlights)) {
    return dashSection('file', 'var(--green)', '重點整理', '尚未產出',
  ...
  if (!b.highlights.length) {
    ...
    return dashSection('file', 'var(--green)', '重點整理', '本輪 0 條',
  ...
  const meta = `<div class="dash-meta">
    ${badge('var(--ac)', `${b.highlights.length} 條重點`)}
    ${b.window_days ? badge('var(--tx3)', `視窗 ${b.window_days} 天`) : ''}
    ...
    ${b.model ? badge('var(--tx3)', `模型 ${esc(String(b.model))}`) : ''}
    ${b.duration_ms ? badge('var(--tx3)', `耗時 ${Math.round(b.duration_ms / 1000)} 秒`) : ''}
    ${srcOn ? badge('var(--red)', '降級輸出（模型失敗）') : badge('var(--purple)', '模型產出')}
    ${g.dropped_dup ? badge('var(--amber)', `去重丟棄 ${g.dropped_dup}`) : ''}
    ${snOn ? badge('var(--red)', '有安全註記') : ''}
  </div>`;

  const items = b.highlights.map((h, i) => {
    const conf = h.confidence || '';
    const cc = DASH_CONF_COLOR[conf] || 'var(--tx3)';
    return `<div class="dash-brief-item">
      <div class="card-row">
        ...
        <div class="card-body">
          ...
          ${h.why_it_matters_zh ? infoBlock('alert', '為何重要', h.why_it_matters_zh) : ''}
          <div class="card-badges">
            ${badge(cc, `信心 ${DASH_CONF[conf] || esc(String(conf))}`)}
            ${(h.confidence_ceiling && h.confidence_ceiling !== conf) ? badge('var(--amber)', `封頂 ${DASH_CONF[h.confidence_ceiling] || esc(String(h.confidence_ceiling))}`) : ''}
            ${badge('var(--ac)', `引用 ${(h.evidence_ids || []).length} 則證據`)}
            ${h.cluster_id ? badge('var(--tx3)', esc(String(h.cluster_id))) : ''}
          </div>
  ...
  return dashSection('file', 'var(--green)', '重點整理', 'BriefWriter 每日精選', meta + items + tail + sec);
}
