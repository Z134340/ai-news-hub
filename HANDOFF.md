...
| **learning-loop v1** | 雙訊號優化迴圈（icon 回饋 × RSS 探索），規範 `docs/specs/learning-loop-v1.md`，施工單 §7 L-0～L-9 | 2026-09-10 拍板並寫入規範；L-0、L-3～L-9 已 commit（2026-09-11），下一步 L-1（需使用者登入點評分）或 L-2；L-5 兩晚驗收與 L-7 實跑驗證待 09-11／09-12 夜跑後看 `.preview/candidates.json`、`.preview/change-eval.json` | — |
