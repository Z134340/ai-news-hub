...
ai-news-hub 端改動＝plist 的 `ProgramArguments` 換成 `run.sh ai-news-hub-daily`；`run-daily.sh` 零改動（2026-09-06 例外：三個前置檢查與整輪 failed 改回 `exit 1`，其餘不動）。
