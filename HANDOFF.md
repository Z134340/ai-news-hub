...
| 2 | 拆成 4 個 session（見 §2）：S2-A canaries + metrics；S2-B run-agents 接線；S2-C search-reviewer scaffold + input；S2-D `newshub_search_reviewer.py` + 08b 接線 | **S2-A 已 commit（折進 `e0f71a4`）；S2-B 已 commit `e43a546`；S2-C 的 2-5 已在磁碟；S2-D 未動工** | S2-A `e0f71a4`（原 `8a2d97a` 懸空）；S2-B `e43a546` |
...
| S2-B | 2-3 `run-agents.sh` 加 `00b-category-metrics`（非阻塞）、S-1 清單、self-test chk | 已 commit `e43a546`（2026-09-04；self-test 42 項 0 失敗、`git diff --check` 乾淨、dry-run 不落地、同日再跑 jsonl 仍 10 行、http 計數 0） | `docs/shapes/run-agents.md` | `bash scripts/run-agents.sh --self-test` 全綠；`git diff --check` 乾淨；commit「Phase 2-B」 |
